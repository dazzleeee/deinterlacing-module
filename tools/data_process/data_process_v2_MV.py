...
# FFmpeg/x264 编码参数
X264_PARAMS = (
    "me=esa:"              # 【重点】使用穷尽搜索(Exhaustive Search)，不再用快速的umh，慢但最准
    "subme=2:"             # 【重点】亚像素搜索只用SAD/SATD，拒绝开启RDO模式决策
    "merange=32:"          # 搜索半径，可以根据你的视频运动幅度调大(如 64)
    "partitions=all:"      # 允许划分到最小的 4x4 块，获得更密集的MV场
    "no-fast-pskip=1:"     # 禁用快速跳过探测，逼迫它必须去搜
    "trellis=0:"           # 关闭网格量化，避免篡改参考帧细节
    "aq-mode=0:"           # 关闭自适应量化，全图一视同仁
    "no-dct-decimate=1:"   # 【新增】不要丢弃微小的残差系数，保留微弱运动的证据
    "no-mbtree=1:"         # 【新增】关闭宏块树时域码率控制，防止未来帧的复杂度影响当前帧的MV
    "no-deblock=1"         # 【新增】关闭去块滤波，防止参考帧的物理边缘被模糊，利于边缘特征匹配
)
FPS = 25  # 帧率
